        ...
        switch (name) {
                    ...
                    content: [
                        {
                            ...
                            text: `# Maven本地仓库扫描结果\n\n找到 ${dependencies.length} 个依赖:\n\n` +
                                dependencies.slice(0, 50).map(dep => `- ${dep.groupId}:${dep.artifactId}:${dep.version}${dep.hasSource ? ' (有源码)' : ''}`).join('\n') +
            ...
            case 'search_class_in_repository': {
                ...
                const resultText = `# 类搜索结果\n\n找到 ${results.length} 个匹配的类:\n\n` +
                    results.slice(0, 20).map(result => `## ${result.fullClassName}\n` +
            ...
            case 'get_dependency_source': {
                const { groupId, artifactId, version, className } = args;
            ...
            case 'get_maven_config': {
                ...
                const configText = `# Maven配置信息\n\n` +
                    ...
                    config.repositories.map(repo => `- **${repo.name || repo.id}**: ${repo.url}`).join('\n') +
                    (Object.keys(config.mirrors).length > 0 ?
                        `\n\n**镜像配置**:\n` +
                            Object.entries(config.mirrors).map(([key, value]) => `- ${key}: ${value}`).join('\n') : '');
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'get_repository_tree': {
                ...
                if (groupId) {
                    ...
                    const treeText = `# 仓库结构 - ${groupId}\n\n` +
                        Object.entries(tree.artifacts).map(([artifactId, info]) => `## ${artifactId}\n- **最新版本**: ${info.latestVersion}\n- **所有版本**: ${info.versions.join(', ')}\n`).join('\n');
                    return {
                        content: [
                            {
                                type: 'text',
            ...
            case 'search_dependencies': {
                ...
                const resultText = `# 依赖搜索结果\n\n` +
                    ...
                    results.slice(0, 30).map(result => `## ${result.groupId}:${result.artifactId}\n` +
