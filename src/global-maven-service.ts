...
import { MavenConfigManager, MavenConfig } from './maven-config.js';
...
export class GlobalMavenService {
  private configManager: MavenConfigManager;
  private decompiler: JavaDecompiler;
  private dependencyCache: Map<string, DependencyInfo[]> = new Map();
  private repositoryTreeCache: Map<string, RepositoryTree> = new Map();
  private dependencyStructureCache: Map<string, DependencyStructure> = new Map();
  ...
  // 从Maven本地仓库扫描所有可用依赖
  async scanLocalRepository(): Promise<DependencyInfo[]> {
  ...
  private async scanDirectory(baseDir: string, currentPath: string, dependencies: DependencyInfo[]): Promise<void> {
  ...
  private async createDependencyInfo(groupId: string, artifactId: string, version: string): Promise<DependencyInfo | null> {
    try {
  ...
  // 搜索指定类名
  async searchClass(className: string): Promise<ClassSearchResult[]> {
  ...
  private async searchInSourcesJar(sourcesJarPath: string, className: string, dependency: DependencyInfo): Promise<ClassSearchResult[]> {
    const results: ClassSearchResult[] = [];
    ...
    try {
      ...
      for (const [filePath, file] of Object.entries(contents.files)) {
        if ((file as any).dir || !filePath.endsWith('.java')) continue;
  ...
  private async searchInJar(jarPath: string, className: string, dependency: DependencyInfo): Promise<ClassSearchResult[]> {
    const results: ClassSearchResult[] = [];
    ...
    try {
      ...
      for (const [filePath, file] of Object.entries(contents.files)) {
        if ((file as any).dir || !filePath.endsWith('.class')) continue;
  ...
  // 获取类的源代码
  async getClassSource(groupId: string, artifactId: string, version: string, className: string): Promise<string> {
  ...
  private async getSourceFromSourcesJar(sourcesJarPath: string, className: string): Promise<string | null> {
    try {
      const data = await fs.readFile(sourcesJarPath);
      const zip = new JSZip();
      const contents = await zip.loadAsync(data);
  ...
  // 列出指定依赖的所有类
  async listClasses(groupId: string, artifactId: string, version: string): Promise<string[]> {
  ...
  private async listClassesFromSourcesJar(sourcesJarPath: string): Promise<string[]> {
    const classes: string[] = [];
  ...
  private async listClassesFromJar(jarPath: string): Promise<string[]> {
    const classes: string[] = [];
  ...
  // 获取依赖信息
  async getDependencyInfo(groupId: string, artifactId: string, version: string): Promise<DependencyInfo | null> {
    return await this.createDependencyInfo(groupId, artifactId, version);
  }

  // 获取Maven配置信息
  async getMavenConfig(): Promise<MavenConfig> {
    return await this.configManager.getConfig();
  }

  // 构建仓库树形结构
  async buildRepositoryTree(): Promise<void> {
    ...
    const groups: { [groupId: string]: RepositoryTree } = {};
  ...
  private async scanRepositoryForTree(baseDir: string, currentPath: string, groups: { [groupId: string]: RepositoryTree }): Promise<void> {
    const fullPath = path.join(baseDir, currentPath);
    const entries = await fs.readdir(fullPath, { withFileTypes: true });
  ...
  // 获取依赖的详细结构
  async getDependencyStructure(groupId: string, artifactId: string, version: string): Promise<DependencyStructure> {
    ...
    const structure: DependencyStructure = {
      groupId,
      artifactId,
      version,
      jarFiles,
  ...
  // 搜索依赖
  searchDependencies(keyword: string): { groupId: string, artifactId: string, versions: string[] }[] {
    const results: { groupId: string, artifactId: string, versions: string[] }[] = [];
  ...
  // 获取依赖源码
  async getDependencySource(groupId: string, artifactId: string, version: string, className: string): Promise<string> {
  ...
  // 从JAR文件中提取源码
  private async extractSourceFromJar(jarPath: string, className: string): Promise<string> {
    const data = await fs.readFile(jarPath);
    const zip = new JSZip();
    const contents = await zip.loadAsync(data);
  ...
  // 在仓库中搜索类
  async searchClassInRepository(className: string): Promise<ClassSearchResult[]> {
    ...
    for (const tree of this.repositoryTreeCache.values()) {
      for (const [artifactId, info] of Object.entries(tree.artifacts)) {
        ...
        try {
          const structure = await this.getDependencyStructure(
            tree.groupId,
            artifactId,
            latestVersion
          );
