...
import { MavenDependencyResolver, ProjectInfo } from './maven-resolver.js';
...
export class JavaDecompiler {
  ...
  async decompileClass(jarPath: string, className: string): Promise<string> {
    try {
      // 获取项目信息（包括Java版本）
      if (!this.projectInfo) {
        try {
  ...
  async searchClassInJar(jarPath: string, searchTerm: string): Promise<string[]> {
  ...
  private async extractClassFromJar(jarPath: string, classFileName: string): Promise<string | null> {
    return new Promise((resolve, reject) => {
      yauzl.open(jarPath, { lazyEntries: true }, (err, zipfile) => {
        if (err) {
          reject(new Error(`无法打开JAR文件: ${err.message}`));
  ...
  private async runEnhancedJavap(jarPath: string, className: string): Promise<string> {
    try {
  ...
  private async runJavapCommand(options: string[]): Promise<string> {
    return new Promise((resolve, reject) => {
  ...
  private async runCfrDecompiler(jarPath: string, className: string): Promise<string> {
    try {
  ...
  private formatSourceCode(sourceCode: string, className: string, jarPath: string, tool: string): string {
...
// 如需更易读的Java源码，CFR反编译器可能无法处理此类`;
  ...
  private async generateFallbackSource(jarPath: string, className: string): Promise<string> {
    try {
...
// 3. JAR包完整且类名正确`;
  ...
  private async generateReadableSource(classPath: string, className: string): Promise<string> {
    try {
      // 简单的类文件分析，提取基本信息
      ...
      let javaVersion = '未知';
      switch (majorVersion) {
        case 45: javaVersion = 'Java 1.1'; break;
        case 46: javaVersion = 'Java 1.2'; break;
        case 47: javaVersion = 'Java 1.3'; break;
        case 48: javaVersion = 'Java 1.4'; break;
        case 49: javaVersion = 'Java 5'; break;
        case 50: javaVersion = 'Java 6'; break;
        case 51: javaVersion = 'Java 7'; break;
        case 52: javaVersion = 'Java 8'; break;
        case 53: javaVersion = 'Java 9'; break;
        case 54: javaVersion = 'Java 10'; break;
        case 55: javaVersion = 'Java 11'; break;
        case 56: javaVersion = 'Java 12'; break;
        case 57: javaVersion = 'Java 13'; break;
        case 58: javaVersion = 'Java 14'; break;
        case 59: javaVersion = 'Java 15'; break;
        case 60: javaVersion = 'Java 16'; break;
        case 61: javaVersion = 'Java 17'; break;
        case 62: javaVersion = 'Java 18'; break;
        case 63: javaVersion = 'Java 19'; break;
        case 64: javaVersion = 'Java 20'; break;
        case 65: javaVersion = 'Java 21'; break;
        default: javaVersion = `Java ${majorVersion - 44}`; break;
      }
...
public class ${className.split('.').pop()} {
  ...
  private async analyzeClassFile(classPath: string): Promise<string> {
    try {
      const classBuffer = await fs.readFile(classPath);
